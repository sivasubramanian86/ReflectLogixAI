...
class BigQueryMCPClient:
    ...
    def query_affect_aggregates(self, user_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "query_executed": f"SELECT mood, AVG(stress_score) FROM `{self.project_id}.analytics.journals` WHERE user_id = '{user_id}' AND created_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL {days} DAY) GROUP BY mood"
        }
