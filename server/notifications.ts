...
import { NotificationConfig, NotificationLog } from '../src/types';
...
export class ExternalNotificationDispatcher {
  public static async dispatchNotification(
    ...
    if (slackUrl && slackUrl.startsWith('http')) {
      try {
        const slackMessage = {
          text: `🌿 *Personal Gemini Journal Alert: ${payload.title}*\n${payload.summarySnippet}\n_Mood: ${payload.moodTag || 'Reflective'} | Actions: ${payload.actionCount || 0}_`,
          mrkdwn: true
        };
        // Simulated HTTPS POST with fetch
        console.log(`[Notification] Dispatching Slack notification to ${slackUrl.substring(0, 25)}...`);
        channels.push('slack');
    ...
    if (discordUrl && discordUrl.startsWith('http')) {
      try {
        console.log(`[Notification] Dispatching Discord notification to ${discordUrl.substring(0, 25)}...`);
